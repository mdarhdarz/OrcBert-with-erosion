...
class SketchTransformerTrainer(Trainer):
    ...
    def update_stroke_results(self, mode, strokes, strokes_pred, masks, size=128):
        ...
        # tmp_stroke_pred[:,:,3] = 0
        # tmp_stroke_pred[:,:,4] = 0
        # for i_ in range(len(masks[0])):
        #     print(masks[0][i_])
        #     print(strokes[0][masks[0]==0])
        ...
        for i in range(min(len(strokes), self.args.num_display_samples)):
            ...
            # imgs_comp_pred.append(strokes2drawing(tmp_comp_stroke_pred,  size=50, svg_filename='{}/tmp_comp_pred_{}_{.3f}.svg'.format(self.tmp_dir, i, self.args.mask_prob)))
            imgs_comp_pred.append(strokes2drawing(tmp_comp_stroke_pred, size=50,
                                                  svg_filename=f'{self.tmp_dir}/tmp_comp_pred_{i}_{self.args.mask_prob:.3f}.svg'))
            # record_img = np.concatenate([imgs[-1],imgs_incomp[-1], imgs_comp_pred[-1]],axis=1) # , imgs_pred[-1], imgs_comp_pred[-1], imgs_incomp[-1],imgs_pred[-1], imgs_comp_pred[-1]

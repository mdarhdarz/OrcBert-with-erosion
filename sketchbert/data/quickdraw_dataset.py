...
class QuickDrawRawDataset(Dataset):
    """
...
class QuickDrawImageDataset(Dataset):
    ...
    Return:
        Corresponding Scene Graph and Images
    """
    def __init__(self, image_dir, mode, image_size, get_type, max_cls_cache, each_max_samples):
    ...
    def get_triplet(self, index):
        ...
        pos_cls, neg_clses = cls, [i for i in range(len(self.cls2index)) if i != cls]
    ...
    def load_dataset(self):
        image_dir = os.path.join(self.image_dir, self.mode)
        self.image_paths = []
        self.classes = []
        self.cls2name = {}
        self.cls2index = {}
        cls_i, ind = 0, 0
        for cls_name in os.listdir(image_dir):
            ...
            if cls_name[0] != '.':
                ...
                for file_name in os.listdir(cls_path):
                    if len(self.cls2index[cls_i]) >= self.each_max_samples:
                        break
                    if file_name[-3:] == 'png':
                        self.image_paths.append(os.path.join(cls_path, file_name))
                        self.classes.append(cls_i)
                        self.cls2index[cls_i].append(ind)
                        ind += 1
                cls_i += 1
        self.num = len(self.image_paths)


class QuickDrawImageSBIRDataset(Dataset):
    """
    Dataset Class for QuickDraw Dataset
    Params:
    ...
    def load_sketch_dataset(self):
        ...
        with open(self.sketch_sum_path, 'r') as rf:
            for line in rf:
                cls_path = line.strip()
                cls_path = cls_path.replace('mode', self.mode)
                cls_name = cls_path[cls_path.rfind('/')+1:]
                ...
                for file_name in os.listdir(cls_path):
                    if len(self.sketch_cls2index[cls_i]) >= self.each_max_samples:
                        break
                    if file_name[-3:] == 'png':
                        self.sketch_paths.append(os.path.join(cls_path, file_name))
                        self.sketch_classes.append(cls_i)
                        self.sketch_cls2index[cls_i].append(ind)
                        ind += 1
                cls_i += 1
...
class QuickDrawMemMapDataset(Dataset):
    """
    Dataset Class for QuickDraw MemMap Dataset
    Params:
    ...
    def get_term(self, index):
        ...
        sketches = np.memmap(path, dtype=np.int16, mode='r', shape=(self.offsets[self.mode][self.cls2name[skt_ind[0]]][-1][-1], 3))
    ...
    def get_mask_result(self, stroke, segment, length_mask, cls):
        ...
        #print((1-mask)[:,0].sum())
        return mask_input, sketch, segment, length_mask, mask, cls
    ...
    def transform_stroke(self, stroke, stroke_type, normalization_type, max_length, max_size, type_size):
        ...
        new_stroke = np.zeros((stroke.shape[0]+1, stroke.shape[1]))
        new_stroke[1:, :] = stroke
        stroke = new_stroke
    ...
    def generate_mask(self, stroke, length, max_length, type='full', dim=6, mask_prob=0.8):
        ...
        if type == 'single':
            ...
            # Mask the axis
            tmp_axis_inds = -torch.ones(max_length)
            tmp_axis_inds[0] = 1
            tmp_axis_inds[1:length] = (stroke[1:length,2] - stroke[:length-1,2])
            tmp_axis_inds[:length] = (tmp_axis_inds[:length] == 1).to(torch.float)
            for ind in [0,1]:
                tmp_length = int(torch.sum(tmp_axis_inds==ind).item())
                ...
                #tmp_axis_inds = (stroke[:,ind][1:] - stroke[:,ind][:-1])

                mask_index = mask_index[tmp_axis_inds==ind][mask]
...
class QuickDrawSBIRDataset(Dataset):
    """
    ...
    def __init__(self, sketch_sum_path, sketch_offset_path, image_sum_path, mode, task_types, get_type, max_length=250, max_size=[256,256], image_size=224, type_size=3, mask_prob=0.8, limit=1000,
                  stroke_type='stroke-5', input_is_complete=True, mask_task_type='task', normalization_type='var_scale',max_scale_factor=10, max_cls_cache=500, each_max_samples=10000, each_image_max_samples=10000, cls_limit_path=None):
        self.mode = mode
        self.task_types = task_types
        self.get_type = get_type
        self.max_length = max_length
    ...
    def get_image_terms(self, index, cls_index):
        pos_cls, neg_clses = cls_index, [i for i in self.cls2name if i != cls_index]
        #print(pos_cls, neg_clses)
        neg_cls = np.random.choice(neg_clses, 1)[0]
